...
c in a hierarchical Generalized Jacobi case.
c
c Input:
...
c    axj, ayj, azj ==> acceleration in Jac. coord
...
c...  Compute acceleration for each orbit k
      do k=2,nbod
            ...
            if (oloc(k,i).eq.1) then
               do j=1, nbod
                  if (oloc(k,j).eq.-1) then ! i = satellite, j = center
                     ...
                     zz = -xx*ir3j(k)*(3.d0*r2(k)*r2(k)+3.d0*r2(k)*xx
     &                  +xx*xx)/(r3(k)+yy)/yy
